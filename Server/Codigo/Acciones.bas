...
If MapData(Map, X, Y).NpcIndex Then
    ...
    If Npclist(MapData(Map, X, Y).NpcIndex).NPCtype = NPCTYPE_ENGARZADOR Then
        ...
        Else
            Call Engarzar(UserIndex, UserList(UserIndex).flags.TargetNpc)
        End If
        Exit Sub
    End If
...
End Sub
Sub AccionParaConsultas(UserIndex As Integer)
 On Error Resume Next
 
 
Dim PathVerCon As String, titCon As String, menCon As String, BaseCon As String, auxcadCon As String
 
PathVerCon = App.Path & "\Consultas\Consulta.ini"

If FileExist(PathVerCon, vbNormal) Then
    Dim numcon As Integer
    numcon = val(GetVar(PathVerCon, "CONSULTAS", "CantCON"))
    BaseCon = Left$(PathVerCon, Len(PathVerCon) - 4)
    Dim loopCon As Integer
    Dim NCon As Integer
    For loopCon = 1 To numcon
        NCon = FreeFile
        PathVerCon = BaseCon & loopCon & ".ini"
        Open PathVerCon For Input Shared As #NCon
        Input #NCon, titCon
        menCon = ""
        auxcadCon = ""
        Do While Not EOF(NCon)
            Input #NCon, auxcadCon
            menCon = menCon & vbCrLf & auxcadCon
        Loop
        Close #NCon
        Call SendData(ToIndex, UserIndex, 0, "CMSG" & titCon & Chr$(176) & menCon)
        
    Next
End If
Call SendData(ToIndex, UserIndex, 0, "VCON")
  End Sub
