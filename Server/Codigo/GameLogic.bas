...
If MapData(Map, X, Y).trigger = 6 Then ' Si paso x este trigger |
If UserList(UserIndex).flags.Muerto = 1 Then ' Si estoy muerto
Call RevivirUsuarioNPC(UserIndex) ' Me revivo
Call SendData(ToPCArea, UserIndex, Map, "TW111")
Call SendData(ToIndex, UserIndex, 0, "||¡Hás sido resucitado!" & FONTTYPE_INFO)
   Call SendData(ToPCArea, UserIndex, UserList(UserIndex).POS.Map, "CFX" & UserList(UserIndex).Char.CharIndex & "," & 36 & "," & 0)
...
If InMapBounds(X, Y) Then
    ...
    If FoundChar = 1 Then
       ...
       If UserList(TempCharIndex).flags.Canje >= 0 Then
            Stat = Stat & " <Puntos de Canje: " & UserList(TempCharIndex).flags.Canje & ">"
        End If
        ...
        End If
            ...
            ElseIf UserList(TempCharIndex).flags.EsConseCaos And UserList(TempCharIndex).Faccion.Bando = Caos Then
                 Stat = Stat & " <Concilio del Caos>~255~60~0~1~0"
            ElseIf UserList(TempCharIndex).flags.EsConseReal And UserList(TempCharIndex).Faccion.Bando = Real Then
                 Stat = Stat & " <Consejo Real>~0~200~255~1~0"
            ElseIf UserList(TempCharIndex).flags.EsConseNeutral And UserList(TempCharIndex).Faccion.Bando = Neutral Then
                 Stat = Stat & " <Coalicion Neutral>~100~100~100~1~0"
            Else
                Stat = Stat & " <NEUTRAL>~180~180~180~1~0"
            End If
