...
Select Case UCase$(Left$(rdata, 2))
    ...
    Case "TI"
        If UserList(UserIndex).flags.Navegando = 1 Or _
           ...
           UserList(UserIndex).POS.Map = 190 Or _
           UserList(UserIndex).POS.Map = 191 Or _
           UserList(UserIndex).POS.Map = 192 Or _
           UserList(UserIndex).POS.Map = 207 Or _
           UserList(UserIndex).POS.Map = 208 Or _
           UserList(UserIndex).POS.Map = 209 Or _
           UserList(UserIndex).POS.Map = 210 Or _
           UserList(UserIndex).POS.Map = 211 Or _
        ...
        If val(Arg1) = FLAGORO Then
            Call SendData(ToIndex, UserIndex, 0, "||No puedes tirar oro." & FONTTYPE_INFO)
            Exit Sub
        Else
...
End Select
 ...
 Select Case UCase$(Left$(rdata, 3))
    Case "DFS" ' VERFPS
         ...
         Call SendData(ToAdmins, UserIndex, 0, "||El Usuario " & UserList(UserIndex).Name & " tiene " & rdata & " FPS por Segundo" & FONTTYPE_INFO)
         Exit Sub
    Case "FRF"
        rdata = Right$(rdata, Len(rdata) - 3)
        For i = 1 To 10
...
Select Case UCase$(Left$(rdata, 5))
    Case "QUEST" ' QUEST
        ...
        Dim Q1 As String
            ...
            Dim Qplayer As Integer
            For Qplayer = 0 To NumQuest
            Q1 = (GetVar(App.Path & "/logs/Quests.log", "PARTICIPANTES", "Part" & Qplayer))
            Call SendData(ToIndex, UserIndex, 0, "VQUE" & Q1)
            Next
        Exit Sub
    
    Case "AMIGO" ' VERIFICACION CLIENTES
    ...
    Case "PARTT" ' TORNEO
        ...
        Dim TOR As String
            ...
            TOR = (GetVar(App.Path & "/logs/torneo.log", "JUGADORES", "JUGADOR" & jugador))
            Call SendData(ToAdmins, 0, 0, "VTOR" & TOR)
            Next
        Exit Sub
    Case "NEWFF"
    ...
    Case "ESTFF" ' ESTADO AMIGOS
        ...
        End If
        Exit Sub
            Case "VENOT" ' VER NOTICIAS
                Dim PathNoticias As String
        
                PathNoticias = App.Path & "\Dat\Noticias.Dat"
                Dim NOT1 As Integer
                Dim Noticia As String
        
                    NOT1 = FreeFile
        
                  Open PathNoticias For Input Shared As #NOT1
                     Input #NOT1, Noticia
                         Close #NOT1
                   Call SendData(ToIndex, UserIndex, 0, "NMSG" & Noticia)
               Exit Sub
    Case "ESNOT" ' ESCRIBIR NOTICIAS
     
        Dim PathNOTI As String
        Dim TextoNOT As String
        Dim NOT2 As String
        rdata = Right$(rdata, Len(rdata) - 5)
        
        TextoNOT = rdata
        PathNOTI = App.Path & "\Dat\Noticias.Dat"
                    
        Dim NOT3 As Integer
        
        If FileExist(PathNOTI, vbNormal) Then Kill (PathNOTI) 'ESCRIBE LA NOTICIA
            NOT3 = FreeFile
            NOT2 = PathNOTI
           Open NOT2 For Output As NOT3
            Print #NOT3, TextoNOT
            Close #NOT3
        Exit Sub
    
    Case "DECON" ' CONSULTAS BORRAR
         rdata = Right$(rdata, Len(rdata) - 5)
         
         Dim NameCOnS As String
         Dim Consus As Integer
         Dim NCop As Integer
         Dim asdConS As String
         Dim auxcadASD As String
         
         If FileExist(App.Path & "\Consultas\Consulta.ini", vbNormal) Then
         NCop = FreeFile
         Dim numConSulin As Integer
         numConSulin = val(GetVar(App.Path & "\Consultas\Consulta.ini", "CONSULTAS", "CantCON"))
         
         For Consus = 1 To numConSulin
             Open App.Path & "\Consultas\Consulta" & Consus & ".ini" For Input Shared As #NCop
                  Input #NCop, NameCOnS
                  asdConS = ""
                  auxcadASD = ""
             Close #NCop
            Kill (App.Path & "\Consultas\Consulta" & Consus & ".ini")
            Call WriteVar(App.Path & "\charfile\" & NameCOnS & ".chr", "CONSULTAS", "consulta", 0)
         Next
            Kill (App.Path & "\Consultas\Consulta.ini")
         Else
         Call SendData(ToIndex, UserIndex, 0, "||No hay consultas para borrar." & FONTTYPE_INFO)
         End If
         
        Exit Sub
    
    Case "VERCO" ' CONSULTAS TEXTO
        Dim PathVerConS As String, menConS As String, BaseConS As String, auxcadConS As String
        rdata = Right$(rdata, Len(rdata) - 5)
        
        PathVerConS = App.Path & "\Consultas\Consulta.ini"

            If FileExist(PathVerConS, vbNormal) Then
               Dim numconS As Integer
               numconS = val(rdata) + 1
               BaseConS = Left$(PathVerConS, Len(PathVerConS) - 4)
               Dim NCons As Integer
                   NCons = FreeFile
                   PathVerConS = BaseConS & numconS & ".ini"
                   Open PathVerConS For Input Shared As #NCons
                   Input #NCons, BaseConS
                        menConS = ""
                        auxcadConS = ""
                   Input #NCons, auxcadConS
                      menConS = menConS & vbCrLf & auxcadConS
               Close #NCons
               Call SendData(ToIndex, UserIndex, 0, "TOCO" & menConS)
            End If
   Exit Sub
    Case "COMSG" ' CONSULTAS
        If val(GetVar(App.Path & "\charfile\" & UserList(UserIndex).Name & ".chr", "CONSULTAS", "consulta")) = 1 Then
             Call SendData(ToIndex, UserIndex, 0, "||Ya has enviado una consulta, espera a que los Administradores la lean." & FONTTYPE_INFO)
             Exit Sub
        End If
        
        Call WriteVar(App.Path & "\charfile\" & UserList(UserIndex).Name & ".chr", "CONSULTAS", "consulta", 1)
        Dim PathConsultas As String
        Dim NombreCon As String
        Dim TextoCon As String
        Dim Con2 As String
        'Dim MotivoCon As String
        rdata = Right$(rdata, Len(rdata) - 5)
        
        'MotivoCon = ReadField(1, rdata, 176)
        NombreCon = ReadField(1, rdata, 176)
        TextoCon = ReadField(2, rdata, 176)
        UserList(UserIndex).flags.EnConsulta = True
        
        PathConsultas = App.Path & "\Consultas\"
        PathConsultas = PathConsultas & "Consulta.ini" ' Removido el MotivoCon
        
        
        
        Dim Con1 As Integer, loopCon As Integer
        
        If FileExist(PathConsultas, vbNormal) Then 'DARLE EN NOMBRE A LA CONSULTA
           Dim numcon As Integer
           numcon = val(GetVar(PathConsultas, "CONSULTAS", "CantCON"))
                    

            Con1 = FreeFile
            Con2 = Left$(PathConsultas, Len(PathConsultas) - 4)
            Con2 = Con2 & numcon + 1 & ".ini"
           Open Con2 For Output As Con1
            'Print #Con1, MotivoCon
            Print #Con1, NombreCon
            Print #Con1, TextoCon
            Call WriteVar(PathConsultas, "CONSULTAS", "CantCON", numcon + 1)
            Close #Con1
           Else
            Con1 = FreeFile
            Con2 = Left$(PathConsultas, Len(PathConsultas) - 4)
           ...
           Open Con2 For Output As Con1
            ...
            Print #Con1, NombreCon
            Print #Con1, TextoCon
            Call WriteVar(PathConsultas, "CONSULTAS", "CantCON", 1)
