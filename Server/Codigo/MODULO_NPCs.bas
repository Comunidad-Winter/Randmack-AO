...
MiNPC = Npclist(NpcIndex)
    ...
    If Npclist(NpcIndex).Name = "Lord Thek" Then
       Call SendData(ToAll, 0, 0, "||Lord Thek > Oh no.. Pense que no podrian conmigo, pero el valiente " & UserList(UserIndex).Name & " me ha derrotado." & FONTTYPE_GUILD)
    End If
    
 'GUERRAFACCIONES
    If PATNPC = 1 Then
       If Npclist(NpcIndex).Name = "TORRENEUT" Then
          Call SendData(ToAll, 0, 0, "||La Torre de los Neutrales ha sido derribada." & FONTTYPE_GUILD)
          For i = 1 To cNeut
              Rdata = GetVar(App.Path & "/logs/guerra.log", "NEUT", "Part" & i)
              tIndex = NameIndex(Rdata)
              Call WarpUserChar(tIndex, 1, 50, 50)
          Next
       ElseIf Npclist(NpcIndex).Name = "TORRECAOS" Then
          Call SendData(ToAll, 0, 0, "||La Torre de las Hordas del Caos ha sido derribada." & FONTTYPE_GUILD)
          For i = 1 To cNeut
              Rdata = GetVar(App.Path & "/logs/guerra.log", "CAOS", "Part" & i)
              tIndex = NameIndex(Rdata)
              Call WarpUserChar(tIndex, 1, 50, 50)
          Next
       ElseIf Npclist(NpcIndex).Name = "TORREREAL" Then
          Call SendData(ToAll, 0, 0, "||La Torre de los Fieles al Rey ha sido derribada." & FONTTYPE_GUILD)
          For i = 1 To cNeut
              Rdata = GetVar(App.Path & "/logs/guerra.log", "REAL", "Part" & i)
              tIndex = NameIndex(Rdata)
              Call WarpUserChar(tIndex, 1, 50, 50)
          Next
       End If
...
If S >= 0 Then
    ...
    Npclist(NpcIndex).GiveEXP = INIDarClaveInt(A, S, "GiveEXP") * 450
    ...
    Npclist(NpcIndex).GiveGLD = INIDarClaveInt(A, S, "GiveGLD") * 450
