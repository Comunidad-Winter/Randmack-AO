...
Item1 = ObjData((MapData(ObjData(ItemIndex).WMapa, ObjData(ItemIndex).WX, ObjData(ItemIndex).WY).trigger = 8)).Name
...
If Item1 = UCase$("Rodela") And Item2 = UCase$("Escudo de Hierro") And Item3 = UCase$("Escudo Imperial") Then
   ...
   Else
      ...
      Call SendData(ToIndex, UserIndex, 0, "||" & vbWhite & "°" & "Felicitaciones!, has obtenido un " & ObjData(MiObj.OBJIndex).Name & "." & "°" & Npclist(NpcIndex).Char.CharIndex & FONTTYPE_TALK)
   End If
End If
...
ElseIf UserList(UserIndex).flags.EsConseReal And UserList(UserIndex).Faccion.Bando = Real Then
    bCr = 7
ElseIf UserList(UserIndex).flags.EsConseNeutral And UserList(UserIndex).Faccion.Bando = Neutral Then
    bCr = 8
...
UserList(UserIndex).Counters.Monster = 0 ' COUNTER MUERTES
Call SendData(ToIndex, UserIndex, 0, "DM" & Map)
...
UserList(UserIndex).Counters.Monster = 0 'MONSTER
...
If UserList(UserIndex).flags.enReto Then
   If UserList(Retos.J1R1).flags.Muerto Then
      Call SendData(ToAll, 0, 0, "||Retos RING1 > " & UserList(Retos.J2R1).Name & " vencio a " & UserList(Retos.J1R1).Name & " en un Reto." & FONTTYPE_VERDEITALICA)
      
      Call WarpUserChar(Retos.J1R1, 1, 50, 50)
      Call WarpUserChar(Retos.J2R1, 1, 50, 50)
      UserList(Retos.J1R1).flags.enReto = False
      UserList(Retos.J2R1).flags.enReto = False
      UserList(Retos.J1R1).flags.EnvReto = False
      UserList(Retos.J2R1).flags.RecReto = False
      Retos.J1R1 = 0
      Retos.J2R1 = 0
      Retos.OCUP1 = False
   ElseIf UserList(Retos.J2R1).flags.Muerto Then
      Call SendData(ToAll, 0, 0, "||Retos RING1 > " & UserList(Retos.J1R1).Name & " vencio a " & UserList(Retos.J2R1).Name & " en un Reto." & FONTTYPE_VERDEITALICA)
      Call WarpUserChar(Retos.J1R1, 1, 50, 50)
      Call WarpUserChar(Retos.J2R1, 1, 50, 50)
      ...
      Retos.J2R1 = 0
      Retos.OCUP1 = False
   ElseIf UserList(Retos.J1R2).flags.Muerto Then
      Call SendData(ToAll, 0, 0, "||Retos RING2 > " & UserList(Retos.J2R2).Name & " vencio a " & UserList(Retos.J1R2).Name & " en un Reto." & FONTTYPE_VERDEITALICA)
      
      Call WarpUserChar(Retos.J1R2, 1, 50, 50)
      Call WarpUserChar(Retos.J2R2, 1, 50, 50)
      UserList(Retos.J1R2).flags.enReto = False
      UserList(Retos.J2R2).flags.enReto = False
      UserList(Retos.J1R2).flags.EnvReto = False
      UserList(Retos.J2R2).flags.RecReto = False
      Retos.J1R2 = 0
      Retos.J2R2 = 0
      Retos.OCUP2 = False
   ElseIf UserList(Retos.J2R2).flags.Muerto Then
      Call SendData(ToAll, 0, 0, "||Retos RING2 > " & UserList(Retos.J1R2).Name & " vencio a " & UserList(Retos.J2R2).Name & " en un Reto." & FONTTYPE_VERDEITALICA)
      Call WarpUserChar(Retos.J1R2, 1, 50, 50)
      Call WarpUserChar(Retos.J2R2, 1, 50, 50)
      UserList(Retos.J1R2).flags.enReto = False
      UserList(Retos.J2R2).flags.enReto = False
...
Dim o As Byte 'Peto -> Declaramos.
UserList(Atacante).flags.TDead = UserList(Atacante).Faccion.Matados(o) + UserList(Atacante).Faccion.Matados(1) + UserList(Atacante).Faccion.Matados(2)
...
Call SendData(ToIndex, Atacante, 0, "FRA" & UserList(Atacante).flags.TDead)

'|||||||||||||| MONSTERKILL! ||||||||||||||
Dim MonWav As Integer
MonWav = UserList(Atacante).POS.Map
If UserList(Muerto).POS.Map = 190 Then ' MONSTER
UserList(Atacante).Counters.Monster = UserList(Atacante).Counters.Monster + 1
   If UserList(Atacante).Counters.Monster = 15 Then
      Call SendData(ToMap, Atacante, MonWav, "HOLY")
      Call SendData(ToMap, Atacante, MonWav, "||" & UserList(Atacante).Name & " es HOLY SHIT!" & FONTTYPE_FIGHT)
   ElseIf UserList(Atacante).Counters.Monster = 12 Then
      Call SendData(ToMap, Atacante, MonWav, "GODL")
      Call SendData(ToMap, Atacante, MonWav, "||" & UserList(Atacante).Name & " es BEYOND GODLIKE!" & FONTTYPE_FIGHT)
   ElseIf UserList(Atacante).Counters.Monster = 10 Then
      Call SendData(ToMap, Atacante, MonWav, "WICK")
      Call SendData(ToMap, Atacante, MonWav, "||" & UserList(Atacante).Name & " es WICKED SICK!" & FONTTYPE_FIGHT)
   ElseIf UserList(Atacante).Counters.Monster = 8 Then
      Call SendData(ToMap, Atacante, MonWav, "MONS")
      Call SendData(ToMap, Atacante, MonWav, "||" & UserList(Atacante).Name & " tiene MONSTER KILL!" & FONTTYPE_FIGHT)
   ElseIf UserList(Atacante).Counters.Monster = 6 Then
      Call SendData(ToMap, Atacante, MonWav, "UNST")
      Call SendData(ToMap, Atacante, MonWav, "||" & UserList(Atacante).Name & " es UNSTOPPABLE!" & FONTTYPE_FIGHT)
   ElseIf UserList(Atacante).Counters.Monster = 4 Then
      Call SendData(ToMap, Atacante, MonWav, "MEGA")
      Call SendData(ToMap, Atacante, MonWav, "||" & UserList(Atacante).Name & " tiene MEGA KILL!" & FONTTYPE_FIGHT)
   ElseIf UserList(Atacante).Counters.Monster = 2 Then
      Call SendData(ToMap, Atacante, MonWav, "KILL")
      Call SendData(ToMap, Atacante, MonWav, "||" & UserList(Atacante).Name & " tiene un KILLING SPREE!" & FONTTYPE_FIGHT)
   End If
End If
'|||||||||||||| MONSTERKILL! ||||||||||||||
End Sub
