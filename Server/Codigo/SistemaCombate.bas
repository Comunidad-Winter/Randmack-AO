...
Else
    Call SendData(ToPCArea, AtacanteIndex, UserList(AtacanteIndex).POS.Map, "||" & vbRed & "°Fallas°" & str(UserList(AtacanteIndex).Char.CharIndex))
    Call SendData(ToPCArea, AtacanteIndex, UserList(AtacanteIndex).POS.Map, "-" & UserList(AtacanteIndex).Char.CharIndex)
...
Call SendData(ToPCArea, AtacanteIndex, UserList(AtacanteIndex).POS.Map, "||" & vbYellow & "°" & Daño & "°" & str(UserList(AtacanteIndex).Char.CharIndex))
...
If MapData(UserList(VictimIndex).POS.Map, UserList(VictimIndex).POS.X, UserList(VictimIndex).POS.Y).trigger = 7 And MapData(UserList(AttackerIndex).POS.Map, UserList(AttackerIndex).POS.X, UserList(AttackerIndex).POS.Y).trigger = 7 Then
    PuedeAtacar = True
    Exit Function
End If
