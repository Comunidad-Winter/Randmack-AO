...
Private Sub tVerClientes_Timer()
   minVC = minVC + 1
    ...
    If minVC = 30 Then
    Call SendData(ToAdmins, 0, 0, "||VC Automatico > Verificando los clientes de los usuarios.." & FONTTYPE_INFO)
    ...
    Next Chitero
    If Chiteros > 0 Then
        Call SendData(ToAdmins, 0, 0, "||VC Automatico > Finalizado. Nro de Clientes Invalidos: " & Chiteros & FONTTYPE_VENENO)
    Else
        Call SendData(ToAdmins, 0, 0, "||VC Automatico > Finalizado. Nro de Clientes Invalidos: 0" & FONTTYPE_VENENO)
    End If
    minVC = 0
    End If
...
If MinutosTLimpiar = 31 Then
Call SendData(ToAll, 0, 0, "||Se realizará una limpieza del Mundo en 60 segundos. Por favor recojan sus items." & FONTTYPE_VENENO)
End If
If MinutosTLimpiar = 32 Then
